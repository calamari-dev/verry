...
class IntervalSeries[T: SignedComparable](Scalar):
    ...
    def __init__(
        self,
        coeffs: Sequence[Interval[T] | T | int | float | str],
    ...
    @overload
    def __call__(self, arg: Interval[T] | T | int | float) -> Interval[T]: ...
    ...
    def __mul__(self, rhs: Self | Interval[T] | T | int | float) -> Self:
        match rhs:
            case self._intvl.endtype() | self._intvl() | int() | float():
    ...
    def __truediv__(self, rhs: Self | Interval[T] | T | int | float) -> Self:
        match rhs:
            case self._intvl.endtype() | self._intvl() | int() | float():
                return self.__class__([x / rhs for x in self.coeffs], intvl=self._intvl)
    ...
    def __radd__(self, lhs: Self | Interval[T] | T | int | float) -> Self:
        return self.__add__(lhs)

    def __rsub__(self, lhs: Self | Interval[T] | T | int | float) -> Self:
        return self.__neg__().__add__(lhs)

    def __rmul__(self, lhs: Self | Interval[T] | T | int | float) -> Self:
        return self.__mul__(lhs)

    def __rtruediv__(self, lhs: Self | Interval[T] | T | int | float) -> Self:
